...
import { User, Mail, Phone, MapPin, Award, CheckCircle2, Bookmark, FileText, Bot, Briefcase, Landmark, ExternalLink, Sparkles, AlertCircle, ShieldCheck } from 'lucide-react';
...
export default function UserDashboard({ subscriptions = { path1: false, path2: false, path3: false }, onSubscribe, onNavigateTab }: UserDashboardProps) {
  ...
  const pathsDetail = [
    ...
    },
    {
      ...
      desc: 'Industry-oriented courses optimized with budget-friendly Indian economy prices.',
      price: '₹3,399/mo',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-950 text-white rounded-2xl p-6 md:p-8 shadow-xl border border-slate-850 relative overflow-hidden">
        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-center">
          {/* Avatar */}
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-tr from-blue-600 to-emerald-400 text-slate-950 flex items-center justify-center font-black text-2xl shadow-lg shrink-0">
            RK
          </div>

          <div className="text-center md:text-left flex-1 space-y-1.5">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              <h2 className="text-xl md:text-2xl font-black">{profile.name}</h2>
              <span className="bg-blue-600/20 text-blue-400 border border-blue-500/30 text-[10px] font-black px-2 py-0.5 rounded-full uppercase self-center md:self-start leading-none mt-1">
                Active Student Profile
              </span>
            </div>

            <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-1 text-slate-400 text-xs font-semibold">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#211b4d] pb-4">
          <div>
            <h3 className="text-sm font-black uppercase tracking-wider text-slate-200 flex items-center gap-2">
              <Sparkles className="w-4.5 h-4.5 text-yellow-300 animate-pulse" /> My Assistance Subscription Plans (₹3,399/Month)
            </h3>
        ...
        {hasAnyActive ? (
          ...
          <div className="bg-[#1a1122] border border-amber-500/20 p-4 rounded-xl flex items-center gap-3 text-amber-200">
            ...
            <div className="text-xs">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">Please subscribe to any path above or on the front page to unlock premium 24/7 priority agent help for ₹3,399/mo.</p>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {savedItems.map((item) => (
                <div key={item.id} className="bg-slate-50 border border-slate-150 p-3.5 rounded-xl text-left">
                  ...
                  <h4 className="font-extrabold text-xs text-slate-800 truncate leading-snug">{item.title}</h4>
